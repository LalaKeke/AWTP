...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        require_exact_port = os.environ.get("CARLA_REQUIRE_EXACT_PORT", "0") == "1"
        if not require_exact_port:
            args.port = find_free_port(args.port)
        carla_server_args = os.environ.get("CARLA_SERVER_ARGS", "-nullrhi -nosound")
        cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} {carla_server_args} -carla-rpc-port={args.port} -graphicsadapter={args.gpu_rank}"
        ...
        while attempts < num_max_restarts:
            ...
            raise RuntimeError(
                "CARLA did not become ready on port {} after {} attempts".format(
                    args.port, num_max_restarts
                )
            )
        attempts = 0
        num_max_restarts = 40
        while attempts < num_max_restarts:
            ...
            raise RuntimeError(
                "Traffic Manager did not become ready on port {} after {} attempts".format(
                    args.traffic_manager_port, num_max_restarts
                )
            )
