...
MONITORED_CONTROL_LOSS_COMPONENT_NAMES = (
    ...
    "loss_actuator_feasibility",
    "loss_risk_early_brake",
    "loss_perception_gated_brake",
    "loss_dipp_control_distill",
    "loss_safe_parent_control_distill",
)
...
class PairedOldNewDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        for k in ["ego_state", "ped_states", "veh_states", "lane_points"]:
...
def compute_perceived_actor_collision_visibility(
    ego_traj: torch.Tensor,
    gt_actor_boxes_2hz: torch.Tensor,
    gt_actor_mask_2hz: torch.Tensor,
    ped_states: torch.Tensor,
    ...
    collision_margin: float = 0.75,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Return GT-collision frames whose actor is observable in HiP-AD input.

    GT supplies supervision only. A collision is learnable when the colliding
    GT box can be matched to at least one actor trajectory that ControlNet
    actually receives. This prevents missed detections from teaching a generic
    braking response with no corresponding input evidence.
    """
    traj_2hz = _sample_trajectory_at_official_2hz(ego_traj)
    perceived_future, perceived_mask = build_agents_future_from_states(
        ped_states,
        veh_states,
        ped_mask,
        veh_mask,
    ...
    if perceived_future.shape[2] <= 6:
        perceived_2hz = perceived_future[:, :, :6, :].transpose(1, 2)
    else:
        frame_indices = [
            min(i, perceived_future.shape[2] - 1)
            for i in (4, 9, 14, 19, 24, 29)
        ]
        perceived_2hz = perceived_future[:, :, frame_indices, :].transpose(1, 2)
    num_frames = min(
        traj_2hz.shape[1],
        gt_actor_boxes_2hz.shape[1],
        perceived_2hz.shape[1],
    )
    num_gt = gt_actor_boxes_2hz.shape[2]
    if num_frames == 0 or num_gt == 0 or perceived_2hz.shape[2] == 0:
        zeros = ego_traj.new_zeros((ego_traj.shape[0], num_frames))
        return zeros, zeros

    gt = gt_actor_boxes_2hz[:, :num_frames]
    gt_mask = gt_actor_mask_2hz[:, :num_frames].bool()
    perceived = perceived_2hz[:, :num_frames]
    center_distance = torch.norm(
        gt[..., :2].unsqueeze(3) - perceived.unsqueeze(2),
        dim=-1,
    )
    matched = (
        center_distance <= float(match_radius)
    ) & perceived_mask[:, None, None, :].bool()
    matched_gt = matched.any(dim=-1) & gt_mask

    per_frame_collision = []
    for frame in range(num_frames):
        actor = gt[:, frame]
        actor_mask = gt_mask[:, frame]
        ego = traj_2hz[:, frame]
        ego_center = ego.new_zeros((ego.shape[0], 4))
        ego_center[:, :2] = ego
        ego_center[:, 0] = ego_center[:, 0] + OFFICIAL_EGO_CENTER_FORWARD_OFFSET
        ego_expand = ego_center.unsqueeze(1).expand(-1, num_gt, -1)
        actor_state = torch.cat(
            [actor[..., :3], torch.zeros_like(actor[..., :1])], dim=-1
        )
        separation, penetration = rect_distance(
            ego_expand.reshape(-1, 4),
            (OFFICIAL_EGO_LENGTH, OFFICIAL_EGO_WIDTH),
            actor_state.reshape(-1, 4),
            (actor[..., 3].reshape(-1), actor[..., 4].reshape(-1)),
        )
        signed = (separation - penetration).reshape(ego.shape[0], num_gt)
        per_frame_collision.append(
            (signed <= float(collision_margin)) & actor_mask
        )
    colliding_gt = torch.stack(per_frame_collision, dim=1)
    collision_gate = colliding_gt.any(dim=-1).to(ego_traj.dtype)
    visible_collision_gate = (
        colliding_gt & matched_gt
    ).any(dim=-1).to(ego_traj.dtype)
    return visible_collision_gate, collision_gate


def compute_deployment_accel_command(
    control: torch.Tensor,
    decay_steps: float = 6.0,
) -> torch.Tensor:
    """Match PNNHorizonController's deployed acceleration fusion exactly."""
    indices = torch.arange(
        control.shape[1], device=control.device, dtype=control.dtype
    )
    weights = torch.exp(-indices / max(float(decay_steps), 1e-3))
    weights = weights / weights.sum().clamp_min(1e-6)
    near_count = min(5, control.shape[1])
    near_weights = weights[:near_count]
    near_weights = near_weights / near_weights.sum().clamp_min(1e-6)
    accel = control[..., 0]
    accel_near = (accel[:, :near_count] * near_weights.unsqueeze(0)).sum(dim=1)
    accel_all = (accel * weights.unsqueeze(0)).sum(dim=1)
    return 0.55 * accel[:, 0] + 0.30 * accel_near + 0.15 * accel_all


def compute_perception_gated_brake_loss_per_sample(
    control: torch.Tensor,
    ego_state: torch.Tensor,
    visible_collision_frames: torch.Tensor,
    max_decel: float = 4.05,
    min_decel: float = 0.5,
    min_speed: float = 1.0,
    reaction_time: float = 0.20,
    decel_gain: float = 0.60,
    decay_steps: float = 6.0,
    time_weights: Optional[Tuple[float, ...]] = None,
) -> torch.Tensor:
    """Penalize deployed positive acceleration only for observable risks."""
    frame_count = visible_collision_frames.shape[1]
    if frame_count == 0:
        return control.new_zeros(control.shape[0])
    accel_command = compute_deployment_accel_command(control, decay_steps)
    speed = ego_state[:, 3].clamp_min(0.0)
    times = 0.5 * torch.arange(
        1, frame_count + 1, device=control.device, dtype=control.dtype
    )
    available_time = (times - float(reaction_time)).clamp_min(0.30)
    required_decel = (
        float(decel_gain) * speed.unsqueeze(1) / available_time.unsqueeze(0)
    ).clamp(min=float(min_decel), max=float(max_decel))
    safe_accel_ceiling = -required_decel
    frame_error = F.relu(
        accel_command.unsqueeze(1) - safe_accel_ceiling
    ).pow(2)
    speed_gate = (speed >= float(min_speed)).to(control.dtype).unsqueeze(1)
    positive_mask = visible_collision_frames.to(control.dtype) * speed_gate
    return positive_normalized_frame_loss_per_sample(
        frame_error,
        positive_mask,
...
def build_pnn_collision_target_weights(
    ...
    # Braking and steering need separate authority. The legacy shared decay
    # relaxed steering whenever an object collision was detected, which could
    # trade an ACR repair for a new lane departure. Keep the legacy value as
    # the fallback so old recipes/checkpoints retain their behavior.
    comfort_decay = float(cfg_runtime.get("weight_pnn_comfort_log_decay", 0.45))
    longitudinal_decay = float(
        cfg_runtime.get("weight_pnn_longitudinal_log_decay", comfort_decay)
    )
    lateral_decay = float(
        cfg_runtime.get("weight_pnn_lateral_log_decay", comfort_decay)
    )
    for name in ("acceleration", "jerk"):
        target[:, idx[name]] *= torch.exp(-longitudinal_decay * any_collision)
    for name in ("steering", "steering_change"):
        target[:, idx[name]] *= torch.exp(-lateral_decay * any_collision)
...
def compute_original_control_loss_with_adaptive_weights(
    ...
    comfort_yaw_rate_threshold: float = 0.95,
    comfort_yaw_accel_threshold: float = 1.93,
    route_track_risk_gate: Optional[torch.Tensor] = None,
    route_track_risk_gain: float = 0.0,
    route_track_min_weight: float = 1.0,
    route_track_lane_risk_gain: float = 0.0,
):
    B, Np, T = u_peds.shape[:3]
    Nv = u_vehs.shape[1]
    ...
    # Sparse route targets describe intent, not a hard obligation to drive
    # through an occupied or invalid corridor. Suppress route tracking only on
    # risky rollouts, while retaining a configurable floor for route progress.
    route_track_gate = torch.ones_like(loss_track_ego_raw)
    if float(route_track_risk_gain) > 0.0:
        if route_track_risk_gate is None:
            actor_risk = (g_safety.detach() > 0.0).to(loss_track_ego_raw.dtype)
        else:
            actor_risk = route_track_risk_gate.detach().reshape(B).to(
                loss_track_ego_raw.dtype
            )
        lane_gain = float(route_track_lane_risk_gain)
        lane_risk = (g_metric_lane.detach() > 0.0).to(loss_track_ego_raw.dtype)
        combined_risk = actor_risk + lane_gain * lane_risk
        floor = min(max(float(route_track_min_weight), 0.0), 1.0)
        route_track_gate = floor + (1.0 - floor) * torch.exp(
            -float(route_track_risk_gain) * combined_risk
        )
    loss_track_ego = loss_track_ego_raw * route_track_gate
    ...
    aux = {
        ...
        "loss_route_track_suppressed": loss_track_ego_raw - loss_track_ego,
...
def augment_sparse_route_targets(
    ego_state: torch.Tensor,
    probability: float = 0.0,
    longitudinal_scale_std: float = 0.0,
    lateral_jitter_std: float = 0.0,
    middle_target_dropout: float = 0.0,
) -> torch.Tensor:
    """Perturb sparse route intent without changing supervision geometry.

    The 10-D ego state stores three route targets in pnn_xy at indices 4:10.
    A shared longitudinal scale and a smooth lateral bias preserve route
    topology. Middle-target dropout replaces only the 2 s point by interpolation
    so it cannot create a discontinuous or zero route target.
    """
    if ego_state.ndim != 2 or ego_state.shape[1] != 10:
        raise ValueError(
            f"expected ego_state [B,10], got {tuple(ego_state.shape)}"
        )
    probability = min(max(float(probability), 0.0), 1.0)
    middle_target_dropout = min(max(float(middle_target_dropout), 0.0), 1.0)
    if probability <= 0.0 and middle_target_dropout <= 0.0:
        return ego_state

    output = ego_state.clone()
    targets = output[:, 4:10].reshape(-1, 3, 2).clone()
    batch = targets.shape[0]
    apply_mask = (
        torch.rand(batch, 1, 1, device=targets.device) < probability
    ).to(targets.dtype)

    if float(longitudinal_scale_std) > 0.0:
        scale = 1.0 + torch.randn(
            batch, 1, 1, device=targets.device, dtype=targets.dtype
        ) * float(longitudinal_scale_std)
        scale = scale.clamp(0.80, 1.20)
        targets[..., 0:1] = targets[..., 0:1] * (
            1.0 + apply_mask * (scale - 1.0)
        )

    if float(lateral_jitter_std) > 0.0:
        lateral_bias = torch.randn(
            batch, 1, 1, device=targets.device, dtype=targets.dtype
        ) * float(lateral_jitter_std)
        ramp = targets.new_tensor((1.0 / 3.0, 2.0 / 3.0, 1.0)).view(1, 3, 1)
        targets[..., 1:2] = targets[..., 1:2] + (
            apply_mask * lateral_bias * ramp
        )

    if middle_target_dropout > 0.0:
        drop_mask = (
            torch.rand(batch, 1, device=targets.device) < middle_target_dropout
        ).to(targets.dtype)
        interpolated = 0.5 * (targets[:, 0] + targets[:, 2])
        targets[:, 1] = (
            (1.0 - drop_mask) * targets[:, 1]
            + drop_mask * interpolated
        )

    output[:, 4:10] = targets.reshape(batch, 6)
    return output
...
def train(rank, world_size, cfg_runtime):
    ...
    if cfg_runtime.get("require_solid_lane_supervision", False):
        required_lane_fields = {"gt_solid_lane_points", "gt_solid_lane_mask"}
        available = dataset.supervision_data or {}
        ...
        lane_mask = available["gt_solid_lane_mask"]
        if lane_mask.numel() == 0 or not bool(lane_mask.any()):
            raise RuntimeError(
                "PNN_REQUIRE_SOLID_LANE_SUPERVISION=1 but all solid-lane masks are empty"
            )
    ...
    policy_core = EluTimeControlEnhanced(
    ...
    if control_ckpt_path:
        ...
        policy_core.load_state_dict(control_ckpt["neural_net"], strict=True)
        if rank == 0:
    ...
    if teacher_ckpt_path:
        ...
        teacher_policy.load_state_dict(teacher_ckpt["neural_net"], strict=True)
    ...
    nnc_model = NeuralNetworkController(policy_core)
    linear_dynamics = BicycleModel(WHEELBASE, dt=DT).to(device)
    nnc_dyn = NNCDynamics(linear_dynamics, nnc_model).to(device)
    ...
    if resume_ckpt_path:
        ...
        if "neural_net" in resume_ckpt:
            policy_core.module.load_state_dict(resume_ckpt["neural_net"], strict=True)
            if rank == 0:
    ...
    decode_min_accel = float(cfg_runtime.get("control_decode_min_accel", -10.0))
    decode_max_accel = float(cfg_runtime.get("control_decode_max_accel", 10.0))
    decode_max_steer = float(cfg_runtime.get("control_decode_max_steer", 1.066))
    if not decode_min_accel < decode_max_accel:
        raise ValueError("control decode acceleration minimum must be below maximum")
    if decode_max_steer <= 0.0:
        raise ValueError("control decode steering limit must be positive")
    a_ego = torch.tensor(
        [decode_min_accel, -decode_max_steer], device=device
    ).view(1, 1, 2)
    b_ego = torch.tensor(
        [decode_max_accel, decode_max_steer], device=device
    ).view(1, 1, 2)
    ...
    for epoch in tqdm(range(start_epoch, cfg_runtime["epochs"]), desc=f"Rank{rank} Training", leave=True):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            ego_state_control = augment_sparse_route_targets(
                ego_state,
                probability=cfg_runtime.get("route_target_aug_probability", 0.0),
                longitudinal_scale_std=cfg_runtime.get(
                    "route_target_longitudinal_scale_std", 0.0
                ),
                lateral_jitter_std=cfg_runtime.get(
                    "route_target_lateral_jitter_std", 0.0
                ),
                middle_target_dropout=cfg_runtime.get(
                    "route_target_middle_dropout", 0.0
                ),
            )
            ego_state_control_n = normalize(ego_state_control, min_ego, max_ego)
            ...
            if clamp_normalized_inputs:
                ...
                ego_state_control_n = ego_state_control_n.clamp(-1.0, 1.0)
            ...
            perceived_collision_frame_gate = torch.zeros_like(
                pnn_collision_frame_gate
            )
            gt_collision_frame_gate_for_visibility = torch.zeros_like(
                pnn_collision_frame_gate
            )
            ...
            if gt_actor_boxes_2hz is not None and gt_actor_mask_2hz is not None:
                ...
                if (
                    cfg_runtime.get(
                        "official_frame_acr_require_perceived_actor", False
                    )
                    or float(
                        cfg_runtime.get("lambda_perception_gated_brake", 0.0)
                    )
                    > 0.0
                ):
                    with torch.no_grad():
                        (
                            perceived_collision_frame_gate,
                            gt_collision_frame_gate_for_visibility,
                        ) = compute_perceived_actor_collision_visibility(
                            ego_traj=ego_metric_traj,
                            gt_actor_boxes_2hz=gt_actor_boxes_2hz,
                            gt_actor_mask_2hz=gt_actor_mask_2hz,
                            ped_states=ped_states,
                            ...
                            veh_mask=veh_mask,
                            rect_distance=safety_loss_module.rect_dist,
                            match_radius=cfg_runtime.get(
                                "perceived_actor_match_radius", 3.0
                            ),
                            collision_margin=cfg_runtime.get(
                                "perceived_actor_collision_margin", 0.75
                            ),
                        )
                        perceived_collision_frame_gate = (
                            perceived_collision_frame_gate
            ...
            # Stage-2 weights are a risk-conditioned residual, not a global
            # replacement for the Stage-1 objective. In gated modes every safe
            # sample uses the exact fixed default weights; only official-geometry
            # actor failures may receive WeightNet's scene-adaptive residual.
            weight_application_mode = str(
                cfg_runtime.get("weight_application_gate_mode", "none")
            ).strip().lower()
            weight_application_gate = torch.ones(
                B, device=device, dtype=ego_metric_traj.dtype
            )
            if weight_application_mode != "none":
                if not use_official_frame_acr:
                    raise ValueError(
                        "weight_application_gate_mode requires "
                        "official_frame_acr_enabled=True"
                    )
                if weight_application_mode == "pnn_object":
                    weight_application_gate = pnn_collision_gate.detach()
                elif weight_application_mode == "pnn_only_object":
                    weight_application_gate = (
                        pnn_collision_gate.detach()
                        * (1.0 - hipad_collision_gate.detach())
                    )
                else:
                    raise ValueError(
                        "weight_application_gate_mode must be one of: "
                        "none, pnn_object, pnn_only_object"
                    )
                gate_col = weight_application_gate.unsqueeze(1)
                cost_weights = default_weights + gate_col * (
                    cost_weights - default_weights
                )
                cost_weights = sanitize_planner_cost_weights(
                    cost_weights,
                    min_weight=cfg_runtime.get(
                        "planner_weight_min_vector",
                        cfg_runtime.get("planner_weight_min", 1e-3),
                    ),
                    max_weight=cfg_runtime.get(
                        "planner_weight_max_vector",
                        cfg_runtime.get("planner_weight_max", 20.0),
                    ),
                    fallback_weights=default_weights,
                    renormalize_to_fallback_sum=False,
                )
                pred_weights_prob = normalize_prob(cost_weights)
                weight_delta = weight_delta * gate_col
            ...
            residual_loss, residual_aux = compute_original_control_loss_with_adaptive_weights(
                ...
                route_track_risk_gate=pnn_collision_gate,
                route_track_risk_gain=cfg_runtime.get(
                    "route_track_risk_gate_gain", 0.0
                ),
                route_track_min_weight=cfg_runtime.get(
                    "route_track_risk_min_weight", 1.0
                ),
                route_track_lane_risk_gain=cfg_runtime.get(
                    "route_track_lane_risk_gain", 0.0
                ),
            )
            ...
            if teacher_metric_traj is not None:
                ...
                trust_risk_gate = (
                    pnn_collision_gate
                    if cfg_runtime.get("teacher_trust_uses_pnn_risk", False)
                    else teacher_risk_gate
                )
            ...
            actuator_feasibility_loss = torch.zeros_like(official_frame_acr_loss)
            risk_early_brake_loss = torch.zeros_like(official_frame_acr_loss)
            perception_gated_brake_loss = torch.zeros_like(
                official_frame_acr_loss
            )
            ...
            if (
                ...
                if cfg_runtime.get(
                    ...
                    frame_mask = frame_mask * perceived_collision_frame_gate[
                        :, :frame_steps
                    ]
                ...
                actuator_weight = float(
                    cfg_runtime.get("lambda_actuator_feasibility", 0.0)
                )
                if actuator_weight > 0.0:
                    max_accel = float(cfg_runtime.get("actuator_max_accel", 2.40))
                    max_decel = float(cfg_runtime.get("actuator_max_decel", 4.05))
                    max_steer = float(cfg_runtime.get("actuator_max_steer", 1.066))
                    accel = init_control[..., 0]
                    steer = init_control[..., 1]
                    actuator_feasibility_loss = actuator_weight * (
                        F.relu(accel - max_accel).pow(2)
                        + F.relu(-max_decel - accel).pow(2)
                        + F.relu(steer.abs() - max_steer).pow(2)
                    ).mean(dim=1)
                    residual_loss = residual_loss + actuator_feasibility_loss

                early_brake_weight = float(
                    cfg_runtime.get("lambda_risk_early_brake", 0.0)
                )
                if early_brake_weight > 0.0:
                    brake_steps = max(
                        1,
                        min(
                            int(cfg_runtime.get("risk_early_brake_steps", 5)),
                            init_control.shape[1],
                        ),
                    )
                    accel_ceiling = float(
                        cfg_runtime.get("risk_early_brake_accel_ceiling", 0.0)
                    )
                    sample_risk = frame_mask.max(dim=1).values
                    positive_accel = F.relu(
                        init_control[:, :brake_steps, 0] - accel_ceiling
                    ).pow(2)
                    early_gate = sample_risk.unsqueeze(1).expand_as(positive_accel)
                    risk_early_brake_loss = early_brake_weight * (
                        positive_normalized_frame_loss_per_sample(
                            positive_accel,
                            early_gate,
                        )
                    )
                    residual_loss = residual_loss + risk_early_brake_loss

                perception_brake_weight = float(
                    cfg_runtime.get("lambda_perception_gated_brake", 0.0)
                )
                if perception_brake_weight > 0.0:
                    perception_gated_brake_loss = (
                        perception_brake_weight
                        * compute_perception_gated_brake_loss_per_sample(
                            control=init_control,
                            ego_state=ego_state,
                            visible_collision_frames=perceived_collision_frame_gate,
                            max_decel=cfg_runtime.get(
                                "actuator_max_decel", 4.05
                            ),
                            min_decel=cfg_runtime.get(
                                "perception_brake_min_decel", 0.5
                            ),
                            min_speed=cfg_runtime.get(
                                "perception_brake_min_speed", 1.0
                            ),
                            reaction_time=cfg_runtime.get(
                                "perception_brake_reaction_time", 0.20
                            ),
                            decel_gain=cfg_runtime.get(
                                "perception_brake_decel_gain", 0.60
                            ),
                            decay_steps=cfg_runtime.get(
                                "perception_brake_decay_steps", 6.0
                            ),
                            time_weights=cfg_runtime.get(
                                "perception_brake_time_weights",
                                (8.0, 8.0, 6.0, 4.0, 2.0, 1.0),
                            ),
                        )
                    )
                    residual_loss = residual_loss + perception_gated_brake_loss
                ...
                if (
                    ...
                    if cfg_runtime.get(
                        "official_frame_lane_guard_include_lane_hits", False
                    ):
                        lane_hit_gate = (
                            pnn_lane_frame_violation[:, :guard_steps]
                            > float(
                                cfg_runtime.get(
                                    "official_frame_lane_guard_self_gate_threshold",
                                    0.0,
                                )
                            )
                        ).to(lane_guard_frame_gate.dtype)
                        lane_guard_frame_gate = torch.maximum(
                            lane_guard_frame_gate, lane_hit_gate
                        )
                    if cfg_runtime.get(
            ...
            residual_aux["loss_actuator_feasibility"] = actuator_feasibility_loss
            residual_aux["loss_risk_early_brake"] = risk_early_brake_loss
            residual_aux[
                "loss_perception_gated_brake"
            ] = perception_gated_brake_loss
            ...
            dipp_control_target = None
            dipp_control_distill_gate = torch.zeros(
                B, device=device, dtype=ego_metric_traj.dtype
            )
            if update_weight:
                ...
                if need_dipp_weight_rollout and update_weight_dipp and not dipp_traj_disabled:
                    ...
                    try:
                        ...
                        # Theseus is absent at deployment, so a useful Stage-2
                        # correction must be transferred into ControlNet. Only
                        # accept targets that turn a PNN-only raw actor hit into
                        # a raw-safe rollout; optionally reject targets that
                        # worsen the metric-aligned solid-lane surrogate.
                        if (
                            float(cfg_runtime.get("lambda_dipp_control_distill", 0.0)) > 0.0
                            and epoch >= int(
                                cfg_runtime.get("dipp_control_distill_start_epoch", 0)
                            )
                            and gt_actor_boxes_2hz is not None
                            and gt_actor_mask_2hz is not None
                        ):
                            with torch.no_grad():
                                dipp_actor_frames = compute_official_actor_raster_proxy_gate(
                                    ego_traj=ego_dipp_metric_traj_w.detach(),
                                    gt_actor_boxes_2hz=gt_actor_boxes_2hz,
                                    gt_actor_mask_2hz=gt_actor_mask_2hz,
                                    sample_valid_mask=official_fut_valid_mask,
                                    frame_valid_mask=gt_obj_frame_valid_mask,
                                ...
                                dipp_actor_safe = 1.0 - dipp_actor_frames.amax(dim=1)
                                accepted = pnn_only_risk_gate.detach() * dipp_actor_safe
                                if cfg_runtime.get(
                                    "dipp_control_distill_require_lane_nonregression", True
                                ):
                                    dipp_lane_check = compute_metric_solid_lane_violation(
                                        ego_traj=ego_dipp_metric_traj_w.detach(),
                                        solid_lane_points=gt_solid_lane_points,
                                        solid_lane_mask=gt_solid_lane_mask,
                                        frame_valid_mask=gt_lane_frame_valid_mask,
                                        ...
                                        margin=cfg_runtime.get("metric_lane_margin", 0.05),
                                    )
                                    lane_tolerance = float(
                                        cfg_runtime.get(
                                            "dipp_control_distill_lane_tolerance", 0.02
                                        )
                                    )
                                    lane_nonregression = (
                                        dipp_lane_check
                                    ...
                                    accepted = accepted * lane_nonregression
                            dipp_control_target = best_control_w.detach()
                            dipp_control_distill_gate = accepted.detach()
                ...
                if dipp_failed_flag.item() > 0.0:
                    dipp_control_target = None
                    dipp_control_distill_gate.zero_()
                    weight_loss_traj = torch.zeros((), device=device, dtype=ego_state.dtype)
                    weight_dipp_safety_loss = torch.zeros((), device=device, dtype=ego_state.dtype)
                    weight_dipp_lane_loss = torch.zeros((), device=device, dtype=ego_state.dtype)
                    weight_dipp_trust_loss = torch.zeros((), device=device, dtype=ego_state.dtype)
            ...
            dipp_control_distill_loss = torch.zeros(
                (), device=device, dtype=ego_state.dtype
            )
            dipp_distill_weight = float(
                cfg_runtime.get("lambda_dipp_control_distill", 0.0)
            )
            if dipp_control_target is not None and dipp_distill_weight > 0.0:
                physical_target = torch.stack(
                    [
                        dipp_control_target[..., 0].clamp(
                            -float(cfg_runtime.get("actuator_max_decel", 4.05)),
                            float(cfg_runtime.get("actuator_max_accel", 2.40)),
                        ),
                        dipp_control_target[..., 1].clamp(
                            -float(cfg_runtime.get("actuator_max_steer", 1.066)),
                            float(cfg_runtime.get("actuator_max_steer", 1.066)),
                        ),
                    ],
                    dim=-1,
                )
                control_error = F.smooth_l1_loss(
                    init_control,
                    physical_target,
                    reduction="none",
                    beta=float(cfg_runtime.get("dipp_control_distill_beta", 0.10)),
                )
                steer_weight = float(
                    cfg_runtime.get("dipp_control_distill_steer_weight", 0.20)
                )
                control_error = (
                    control_error[..., 0] + steer_weight * control_error[..., 1]
                )
                thirds = torch.as_tensor(
                    cfg_runtime.get(
                        "dipp_control_distill_time_weights", (3.0, 2.0, 1.0)
                    ),
                    device=device,
                    dtype=control_error.dtype,
                )
                segment_index = torch.div(
                    torch.arange(control_error.shape[1], device=device) * 3,
                    control_error.shape[1],
                    rounding_mode="floor",
                ).clamp_max(2)
                time_weights = thirds[segment_index]
                error_per_sample = (
                    control_error * time_weights.unsqueeze(0)
                ).sum(dim=1) / time_weights.sum().clamp_min(1e-6)
                global_positive_count = reduce_sum_tensor(
                    dipp_control_distill_gate.sum()
                ).clamp_min(1.0)
                dipp_control_distill_loss = (
                    dipp_distill_weight
                    * float(world_size)
                    * (dipp_control_distill_gate * error_per_sample).sum()
                    / global_positive_count
                )

            safe_parent_control_distill_loss = torch.zeros(
                (), device=device, dtype=ego_state.dtype
            )
            safe_control_weight = float(
                cfg_runtime.get("lambda_safe_parent_control_distill", 0.0)
            )
            if teacher_u_ego is not None and safe_control_weight > 0.0:
                teacher_obj_gate = teacher_collision_frame_gate.amax(dim=1)
                teacher_lane_gate = teacher_lane_collision_frame_gate.amax(dim=1)
                teacher_any_gate = torch.maximum(teacher_obj_gate, teacher_lane_gate)
                safe_control_gate = (
                    1.0 - torch.maximum(weight_any_collision_gate, teacher_any_gate)
                ).detach()
                parent_target = torch.stack(
                    [
                        teacher_u_ego[..., 0].clamp(
                            -float(cfg_runtime.get("actuator_max_decel", 4.05)),
                            float(cfg_runtime.get("actuator_max_accel", 2.40)),
                        ),
                        teacher_u_ego[..., 1].clamp(
                            -float(cfg_runtime.get("actuator_max_steer", 1.066)),
                            float(cfg_runtime.get("actuator_max_steer", 1.066)),
                        ),
                    ],
                ...
                parent_error = F.smooth_l1_loss(
                    init_control,
                    parent_target,
                    reduction="none",
                    beta=float(
                        cfg_runtime.get("safe_parent_control_distill_beta", 0.10)
                    ),
                )
                parent_error = (
                    parent_error[..., 0]
                    + float(
                        cfg_runtime.get(
                            "safe_parent_control_distill_steer_weight", 1.0
                        )
                    )
                    * parent_error[..., 1]
                ).mean(dim=1)
                global_safe_count = reduce_sum_tensor(
                    safe_control_gate.sum()
                ).clamp_min(1.0)
                safe_parent_control_distill_loss = (
                    safe_control_weight
                    * float(world_size)
                    * (safe_control_gate * parent_error).sum()
                    / global_safe_count
                )

            residual_aux["loss_dipp_control_distill"] = dipp_control_distill_loss
            residual_aux[
                "loss_safe_parent_control_distill"
            ] = safe_parent_control_distill_loss
            ...
            if writer is not None:
                ...
                visible_collision_count = perceived_collision_frame_gate.sum()
                gt_collision_count = pnn_collision_frame_gate.sum().clamp_min(1.0)
                writer.add_scalar(
                    "acr/frame_perceived_collision_rate",
                    perceived_collision_frame_gate.mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "acr/perceived_collision_coverage",
                    (visible_collision_count / gt_collision_count).item(),
                    global_step,
                )
                writer.add_scalar(
                    "acr/frame_unobservable_collision_rate",
                    (
                        pnn_collision_frame_gate
                        * (1.0 - perceived_collision_frame_gate)
                    ).mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "control/deployment_accel_command_mean",
                    compute_deployment_accel_command(
                        init_control,
                        cfg_runtime.get("perception_brake_decay_steps", 6.0),
                    ).mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "dipp/control_distill_accept_rate",
                    dipp_control_distill_gate.mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "weight/application_gate_rate",
                    weight_application_gate.mean().item(),
                    global_step,
                )
                if use_official_frame_acr:
                    writer.add_scalar(
            ...
            if rank == 0 and batch_idx % 100 == 0:
                ...
                print(
                    ...
                    f"dipp_accept={dipp_control_distill_gate.mean().item() * 100:.2f}% | "
...
def main():
    ...
    cfg_runtime = {
        ...
        "save_dir": os.path.join(PROJECT_ROOT, "outputs", "pnn_train_v10"),
