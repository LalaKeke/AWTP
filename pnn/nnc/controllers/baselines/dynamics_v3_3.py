...
class BicycleModel(ControlledDynamics):
    ...
    def forward(self,
        ...
        copy_x_theta = x_theta.clone()
        # 计算状态变化量 dx
        # dx_pos = (v * torch.cos(theta))
        # dy_pos = (v * torch.sin(theta))
        time_dx = time.time()
        dx_pos = (copy_x_v * torch.cos(copy_x_theta))

        time_dy = time.time()
        print('time_dx', time_dy - time_dx)
        dy_pos = (copy_x_v * torch.sin(copy_x_theta))
        time_dtheta = time.time()
        print('time_dy', time_dtheta - time_dy)
        dtheta = (x_v / self.wheelbase) * torch.tan(u_delta)
        time_dv = time.time()
        print('time_dtheta', time_dv - time_dtheta)
        dv = u_a
        time_end = time.time()
        print('time_dv', time_end - time_dv)
        ...
        return dx_pos, dy_pos, dtheta, dv