...
class SparsePlanDecoder(object):
        ...
        for i in range(batch_size):
                ...
                if anchor_type[0] in ["temp", "spat"]:
                    ...
                    if anchor_type[1] not in speed_dict:
                        speed_dict[anchor_type[1]] = {
                            "cls_preds": [cls_preds[idx][i:i + 1]],
                            "reg_preds": [reg_preds[idx][i]],
                            "speed_areas": [anchor_type[2]]
                        }
                    else:
                        speed_dict[anchor_type[1]]["cls_preds"].append(cls_preds[idx][i:i + 1])
            ...
            if len(speed_dict):
                ...
                if self.with_rescore and len(det_output["prediction"]) > 0 and len(motion_output["prediction"]) > 0:
                    speed_dict = self.rescore_speed(speed_dict, det_output, motion_output, i)
    ...
    def rescore_speed(self, speed_dict, det_output, motion_output, batch_index):
        # Speed plans are decoded one sample at a time in post_process. Keep
        # detection and motion tensors on that same sample to avoid comparing
        # one ego plan with agents from every item in the batch.
        det_classification = det_output["classification"][-1][batch_index:batch_index + 1].sigmoid()
        det_anchors = det_output["prediction"][-1][batch_index:batch_index + 1]
        det_confidence = det_classification.max(dim=-1).values

        motion_cls = motion_output["classification"][-1][batch_index:batch_index + 1].sigmoid()
        motion_reg = motion_output["prediction"][-1][batch_index:batch_index + 1].cumsum(-2)
