...
class PlanningInstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas, feature_maps, dn_metas=None):
        ...
        if self.cached_anchor is not None and batch_size == self.cached_anchor.shape[0]:
            ...
            # The distributed sampler can produce a smaller final batch. Cached
            # temporal instances from the preceding full batch cannot be
            # reshaped or matched to it, so start that batch without history.
            self.reset()
