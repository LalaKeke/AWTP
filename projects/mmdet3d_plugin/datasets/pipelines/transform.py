...
class NuScenesSparse4DAdaptor(object):
    ...
    def __call__(self, input_dict):
        ...
        for key in [
            ...
            'gt_agent_fut_trajs',
            'gt_agent_fut_masks',
            # Number of detected GT agents differs per sample. The attribute
            # tensor is [num_agents, 34], so it must stay as a per-sample list
            # when evaluation uses samples_per_gpu > 1.
            'gt_attr_labels',
        ]:
