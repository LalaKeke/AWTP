...
def masked_agent_minmax(states, mask, q_low, q_high, feature_dim):
...
def main():
    ...
    min_ped, max_ped = masked_agent_minmax(
        data["ped_states"], data.get("ped_mask"), args.q_low, args.q_high, 6
    )
    min_veh, max_veh = masked_agent_minmax(
        data["veh_states"], data.get("veh_mask"), args.q_low, args.q_high, 6
    )
    min_static, max_static = masked_agent_minmax(
        data["static_states"],
        data.get("static_mask"),
        args.q_low,
        args.q_high,
        12,
    )
    ...
    output = {
        "format": "pnn_static_precomputed_normalization_v1",
        "q_low": args.q_low,
        "q_high": args.q_high,
        "min_ego": min_ego,
        "max_ego": max_ego,
