...
import convert_hipad_chunk_to_pnn_pt as base
...
def _convert_chunk(chunk_id: int):
    ...
    if _CFG.resume and os.path.exists(old_path) and os.path.exists(new_path):
        ...
        required = {"gt_actor_boxes_2hz", "gt_actor_mask_2hz", "official_hipad_obj_box_col"}
        if required.issubset(new):
...
def parse_args():
    ...
    parser.add_argument("--old-name", default="hipad_stage2_b2d_train_pnn_old.pt")
    parser.add_argument("--new-name", default="hipad_stage2_b2d_train_pnn_true_gt.pt")
