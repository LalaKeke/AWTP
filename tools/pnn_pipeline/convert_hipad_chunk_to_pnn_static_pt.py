...
VEH_LABELS = {0, 1, 2, 3}
PED_LABELS = {7}
STATIC_LABELS = {4, 5, 6, 8}
STATIC_LABEL_TO_SLOT = {4: 0, 5: 1, 6: 2, 8: 3}
MAX_STATIC_OBJECTS = 24
...
def build_coord_meta(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {
        ...
        "static_state_layout": (
            "static_states=[x,y,z,yaw,length,width,height,score,"
            "is_sign,is_cone,is_light,is_other]; source=HiP-AD predicted boxes only"
        ),
...
def extract_agents(
    img_bbox: Mapping[str, Any],
    score_thr: float = 0.3,
    actor_motion_source_dt: float = HIPAD_MOTION_DT,
    ...
    for box, label, future, score in zip(boxes, labels, trajs, kept_scores):
        label = int(label)
        if label in STATIC_LABELS:
            # HiP-AD boxes use (x,y,z,width,length,height,yaw,...).
            if box.shape[0] < 7:
                continue
            static_objects.append(
                {
                    ...
                    "z": float(box[2]),
                    "yaw": float(box[6]),
                    "length": max(float(box[4]), 0.05),
                    "width": max(float(box[3]), 0.05),
                    "height": max(float(box[5]), 0.05),
                    "score": float(score),
                    "label": label,
                }
            )
        ...
        aligned_future = align_hipad_motion_future(
            current_xy=box[:2],
            future=future,
            source_dt=actor_motion_source_dt,
            measured_velocity_xy=velocity_xy,
            max_speed=5.0 if label in PED_LABELS else 20.0,
        )
    ...
    return ped_agents, veh_agents, static_objects


def pack_static_objects(
    objects: Sequence[Mapping[str, Any]],
    max_objects: int = MAX_STATIC_OBJECTS,
) -> Tuple[np.ndarray, np.ndarray]:
    """Pack HiP-AD static detections without introducing GT-only fields."""
    states = np.zeros((max_objects, STATIC_FEATURE_DIM), dtype=np.float32)
    mask = np.zeros((max_objects,), dtype=np.bool_)
    ordered = sorted(
        objects,
        key=lambda item: float(np.hypot(item["x"], item["y"])),
    )
    for slot, item in enumerate(ordered[:max_objects]):
        label = int(item["label"])
        one_hot = np.zeros((4,), dtype=np.float32)
        one_hot[STATIC_LABEL_TO_SLOT[label]] = 1.0
        states[slot] = np.asarray(
            [
                item["x"],
                item["y"],
                item["z"],
...
def make_sample(
    ...
    ped_agents, veh_agents, static_objects = extract_agents(
        img_bbox,
        score_thr=score_thr,
        actor_motion_source_dt=actor_motion_source_dt,
    )
    ped_states, ped_mask = pack_agents(ped_agents, NUM_PEDS)
    veh_states, veh_mask = pack_agents(veh_agents, NUM_VEHS)
    static_states, static_mask = pack_static_objects(static_objects)
    ...
    if coord_convention == "pnn_xy":
        ...
        static_states[..., 0:2] = hipad_points_to_pnn(
            static_states[..., 0:2]
        )
        static_states[..., 3] = hipad_yaw_to_pnn(static_states[..., 3])
    ...
    hipad_plan_2hz = np.asarray(plan, dtype=np.float32)
    if coord_convention == "pnn_xy":
        hipad_plan_2hz = hipad_points_to_pnn(hipad_plan_2hz)
    return {
        "ego_state": ego_state,
...
def convert_records(
    ...
    old_data = {
        ...
        "static_states": torch.as_tensor(np.stack([s["static_states"] for s in samples]), dtype=torch.float32),
        ...
        "static_mask": torch.as_tensor(np.stack([s["static_mask"] for s in samples]), dtype=torch.bool),
    }
    new_data = {
        ...
        "hipad_plan_2hz": torch.as_tensor(np.stack([s["hipad_plan_2hz"] for s in samples]), dtype=torch.float32),
...
def convert(args: argparse.Namespace) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
    ...
    old_data, new_data = convert_records(
        ...
        actor_motion_source_dt=args.actor_motion_source_dt,
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default=os.path.join(HIPAD_ROOT, "data/pnn/static_v1"))
    ...
    parser.add_argument(
        "--actor-motion-source-dt",
        type=float,
        default=HIPAD_MOTION_DT,
        help="HiP-AD trajs_3d sampling interval; Bench2Drive stage2 uses 0.1 s",
    )
